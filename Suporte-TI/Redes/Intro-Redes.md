...
Computadores se comunicam entre si como humanos.
...
E a camada de aplicação é o conteúdo do pacote em si, como o vestido que você comprou ou o livro que você encomendou.

## O modelo de rede OSI

Além do modelo de cinco camadas que vamos ver, é importante saber que existem outros modelos. O modelo TCP/IP tradicional tem apenas quatro camadas, já que não diferencia a camada física da camada de enlace de dados, mas é, por sua vez, muito semelhante com o que vamos estudar. O outro modelo mais conhecido é o OSI. Esse é o modelo ensinado em muitos outros programas de certificação de rede, como o Net+ e as certificações de rede da Cisco. A principal diferença entre o nosso modelo de cinco camadas e o modelo OSI de sete camadas é que o modelo OSI divide a camada de aplicação em três camadas no total.

Você pode ler mais sobre o Modelo de rede OSI na Wikipédia clicando [aqui](https://en.wikipedia.org/wiki/OSI_model).

## Visão geral dos dispositivos de red
...
Aqui está um resumo mais casual e acessível do parágrafo:

Os cabos de rede se conectam aos dispositivos usando um conector RJ45.

As portas de rede geralmente têm dois LEDs que mostram se o cabo está conectado corretamente e se há dados sendo transmitidos.

Em alguns casos, as portas de rede são instaladas na parede ou embaixo da mesa e são conectadas a um patch panel.

O patch panel é usado para conectar os cabos de rede aos switches ou roteadores.
